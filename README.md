...
#### `str_t     string_lower    (const string_t self)`
...
Value       | Meaning
:-----------|:--------------------------------------------
`BEGIN`     | operate from the first character on forwards
`END`       | operate from the last character on backwards
`WITHIN`    | operate from a position inside the string on
